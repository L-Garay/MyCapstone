...
<div class="continer-fluid">
  ...
  <div class="row">
    <div class="col">
      <h1>{{outing.name}}
        <i @click.prevent="deleteOuting(outing)" style="color:red" class="fas fa-trash"></i>     
        <i @click.prevent="showEditOutingModal" style="color:green" class="fas fa-edit"></i></h1>
      <h1>{{outing.date}}</h1>
    </div>
  </div>
...
export default {
  ...
  computed: {
    outing() {
      ...
      return (
        this.$store.state.outings.find(o => o._id == this.$route.params.outingId) || {
          title: "Loading..."
        }
      );
    },
